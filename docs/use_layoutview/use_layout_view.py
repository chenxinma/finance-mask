...
def classify_excel_sheets(xlsx_path):
    ...
    """
    lib = load_rust_library(Path(__file__).parent.parent.parent.parent)
...
# Usage
# 调用分类函数
# sheets = classify_excel_sheets(f_path)
# if not sheets:
#     logger.warning(f"No sheets found in {metadata.original_filename}")
#     metadata.mark_error("No sheets found", self.name)

# 为每个工作表确定类型Data or Form
# for sheet_info in sheets:
#     sheet_type = sheet_info["sheet_type"] # Literal["Data", "Form"]